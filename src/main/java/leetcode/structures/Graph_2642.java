...
public class Graph_2642 {
    ...
    public Graph_2642(final int n, final int[][] edges) {
    ...
    public int shortestPath(final int node1, final int node2) {
        final int n = adjList.size();
        final var pq = new PriorityQueue<List<Integer>>(Comparator.comparingInt(list -> list.get(0)));
        final int[] costForNode = new int[n];
        ...
        while (!pq.isEmpty()) {
            ...
            for (var neighbor : adjList.get(currNode)) {
                int neighborNode = neighbor.getLeft();
                int cost = neighbor.getRight();
