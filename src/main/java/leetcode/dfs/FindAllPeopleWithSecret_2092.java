...
public class FindAllPeopleWithSecret_2092 {
    ...
    //Time O(M * log(M+N)) and Space O(M + N) where N -number of people and M - number of meetings
    public List<Integer> findAllPeople(int n, int[][] meetings, int firstPerson) {
        // Sort meetings in increasing order of time
        Arrays.sort(meetings, (a, b) -> a[2] - b[2]);

        // Group Meetings in increasing order of time
        final Map<Integer, List<int[]>> sameTimeMeetings = new TreeMap<>();
        ...
        final boolean[] knowsSecret = new boolean[n];
        knowsSecret[0] = true;
        ...
        for (int t : sameTimeMeetings.keySet()) {
            ...
            final Map<Integer, List<Integer>> meet = new HashMap<>();
            ...
            final Set<Integer> start = new HashSet<>();
            for (int[] meeting : sameTimeMeetings.get(t)) {
                int x = meeting[0], y = meeting[1];
                if (knowsSecret[x]) {
                    start.add(x);
            ...
            // Do BFS
            final Queue<Integer> q = new LinkedList<>(start);
            while (!q.isEmpty()) {
                int person = q.poll();
