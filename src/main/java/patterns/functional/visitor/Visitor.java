...
public interface Visitor<R> {
    ...
    static <R> Visitor<R> of(Consumer<VisitorBuilder<R>>  consumer) {
        final Map<Class<?>, Function<Object, R>> registry = new HashMap<>();
        final VisitorBuilder<R> visitorBuilder = new VisitorBuilder<R>() {
            @Override
            public <T> void registry(Class<T> type, Function<T, R> function) {
    ...
    interface X<T, R> {
        ...
        default Y<R> execute(Function<T, R> function) {
            return visitorBuilder -> visitorBuilder.registry(type(), function);
        }
    }
    ...
    interface Z<T, R> {
        ...
        default Y<R> execute(Function<T, R> function) {
            return previousConsumer().andThen(visitorBuilder -> visitorBuilder.registry(type(), function));
        }
